...
- 사용자와 직접 소통하는 역할은 PM뿐이다. 요청 접수, 질문, 진행 보고, 승인 요청, 최종 결과 전달을 PM이 담당한다.
- 다른 역할은 질문·진행·완료·차단 사유를 PM에게 보고한다. 사용자에게 직접 입력이나 역할 간 메시지 전달을 요구하지 않는다.
- 역할 간 기술 협의는 가능하지만 사용자 결정이 필요한 사항은 PM이 취합한다. 운영체제·CLI의 권한 승인 절차를 우회하지 않는다.
- QA도 예외가 아니다. 현재 QA 통신 연결이 없으면 PM이 차단 사유로 관리하며, 사용자에게 QA 세션에서 대신 전달하도록 요구하지 않는다.
- 웹 UI의 비PM 역할 화면은 관찰용이며 사용자 대화 창구는 PM이다.
...
[기술스택](architecture/TECH_STACK.md)은 웹 프로젝트 관리와 공유하는 원본이다. PM과 구현 역할은 새 작업 시작 전에 다시 읽고 확정된 기술·버전·제약을 따른다. 미정 항목은 PM이 사용자와 확인한다. 문서 저장은 패키지 설치나 기존 코드 마이그레이션 승인이 아니다.

디자인 요청의 원본은 [디자인 프롬프트](product/DESIGN_PROMPT.md)다. PM과 디자이너는 디자인 작업 전에 다시 읽고 화면·스타일·참고 링크·제약을 반영한다.
