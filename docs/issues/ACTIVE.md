...
### ISSUE-0001 수정 — URL 연결 확인 및 웹 문서 편집

- URL 저장 후 제한 시간 내 HTTP HEAD 응답 확인, 로그인·이동·비공개 가능성·연결 오류 구분. 자동 리디렉션 및 인증정보 전송 없음.
- 허용된 문서만 웹 Markdown 편집·저장, CSRF 검사와 내용 버전 비교로 오래된 저장 거부, 임시 파일 교체 저장. 미저장 입력의 이동·닫기 확인 및 저장 오류 시 내용 보존.
- Python 테스트 48개 통과, JavaScript 구문 검사 통과. 실제 외부 비공개 링크 인증은 검증 범위 제외.

### ISSUE-0001 수정 — 문서 업데이트 빨간 점 알림

- 문서 내용 해시를 5초마다 비교하고 프로젝트별 브라우저 읽음 기록 유지. 처음 접속 시 현재 문서를 기준으로 삼고 이후 추가·수정된 문서에 알림. 해당 문서를 정상 조회한 경우만 읽음 처리, 편집 내용을 자동 교체하지 않음.
- 문서 테스트 7개 통과, 동일 수정 시각의 내용 변경 감지 포함. 임시 서버 브라우저에서 외부 파일 변경 → 업데이트 1개 표시 → 열람 → 알림 해제 확인.

### ISSUE-0001 수정 — 기술스택 관리

- 프로젝트 관리의 기술스택 버튼·탭·Markdown 편집 폼 추가. 공용 TECH_STACK.md를 단일 원본으로 읽고 저장, 문서 편집과 충돌 감지 공유. 새 프로젝트 템플릿 포함.
- PM·구현 역할이 새 작업 전에 기술스택을 읽도록 진입 문서 및 초기 지시 보완. 저장 즉시 문서·변경 알림에 반영하며 실행 중인 역할에 강제 메시지나 패키지 설치는 하지 않음.
- Python 51개 테스트 통과, 기술스택 생성·문서 양방향 수정·오래된 버전 저장 거부·링크 경로 거부 포함. JS 구문 검사 통과.

### ISSUE-0001 수정 — 디자인 프롬프트 입력

- 프로젝트 관리 버튼·탭에 디자인 프롬프트 추가. Markdown 입력·저장·다시 불러오기, 공용 DESIGN_PROMPT.md와 양방향 반영 및 버전 충돌 검사. 신규 프로젝트 템플릿과 PM·디자이너 참고 지시 포함.
- Python 테스트 52개 통과, 디자인 문서 생성·수정·기술스택과의 분리·충돌 거부 검증. JavaScript 구문 검사 통과.
