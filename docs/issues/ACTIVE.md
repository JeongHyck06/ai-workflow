...
### ISSUE-0001 수정 — 기본 스킬 링크 초기화 오류

- 저장소의 .agents/skills/setup/SKILL.md는 .claude/skills/setup/SKILL.md를 가리키는 정상 내부 링크인데 초기화가 모든 링크를 차단하던 오류 수정.
- 스킬 대상 경로를 초기화 전 검증. 도구 내부 링크는 보존하고 실제 대상에만 누락 파일 생성. 외부 링크·순환 링크·파일이 아닌 대상은 거부.
- 이전 버전이 백업·문서 생성 이후 실패한 경우 기존 백업과 현재 프로젝트 문서를 보존하며 재실행 가능.
- 전체 테스트 38개 통과: 배포 스킬 링크 확인, 내부 링크·누락 대상 생성, 실패 후 재시작, 외부 링크의 변경 전 차단 포함. 원본 저장소만 수정, 커밋·푸시 미실행.


### ISSUE-0001 수정 — 제품 app·backend 위치 확인

- 사용자 명확화에 따라 프론트엔드 제품 폴더명을 frontend에서 app으로 변경. 역할 이름 FRONTEND는 유지.
- app과 backend는 제품 루트의 ai-workflow 바깥에 생성하며, 협업 파일은 도구 안에 유지. 초기 지시·초기 문서 경로·사용법·경로 회귀 테스트도 같은 구조로 변경.
- 원본 저장소만 수정. 기존 프로젝트 폴더나 파일을 이동·삭제하지 않음.


### ISSUE-0001 수정 — ZIP 이름에서 제품 루트 오인

- ai-workflow-main처럼 ZIP 폴더명이 달라지면 도구 자체를 제품 루트로 잡던 조건 제거. 기본값은 항상 도구의 상위 경로이며 저장된 정상 프로젝트 경로·명시적 --project는 존중.
- 구버전의 도구 자기 참조 설정은 상위로 교정하고 app·backend를 이동. 내부/외부 모두 코드가 있으면 전체 이동 전에 중단해 원본 보존. 명시적으로 도구 자체를 제품으로 선택한 경우는 별도 플래그로 구별.
- 41개 테스트 통과. 공백이 포함된 Test Todo/ai-workflow-main, 임의 ZIP 폴더명, 잘못된 설정 복구, 코드 이동·재실행·충돌 보존 포함. 원본 저장소만 수정, 사용자 사본은 읽기 확인만 수행.

### ISSUE-0001 수정 — 여러 프로젝트의 웹·세션 구분 (2026-09-21)

- 원인 확인: 8765 웹은 원본 ai-workflow 서버였으며 Test Todo의 런타임 세션 상태 파일은 없었음. 고정 포트 사전 검사 실패 시 팀 실행에 도달하지 못하는 구조를 수정.
- 웹을 먼저 시작하고 팀 초기화는 백그라운드 실행. 8765부터 빈 포트를 실제 바인딩해 배정하고 같은 도구 사본은 파일 잠금으로 중복 모니터를 방지, 기존 주소를 안내. 명시적 --port는 자동 변경하지 않음.
- 공통 vive 접두사 제거, 모든 프로젝트 절대 경로로 세션 이름 구분. 저장된 타 프로젝트 세션 로그를 조회하지 않으며 웹에 전체 프로젝트 경로와 팀 초기화 오류 표시.
- 검증: Python 회귀 테스트 43개 통과. 실제 임시 프로젝트 두 개 동시 실행, API의 제품 루트 분리, 포트 분리, 반복 실행 재사용 검증. 실제 모델 세션 실행·Browser QA는 미실행. 사용자 프로젝트 사본과 기존 서버는 수정하지 않음.

### ISSUE-0001 수정 — 제품별 Git 저장소 및 PM 질문 알림

- 사용자 요청: backend·frontend·app 세 폴더를 독립 로컬 Git 저장소로 초기화. 기존 저장소·origin 보존, 원격 생성과 push는 수행하지 않음. PR 규칙 유지.
- 프로젝트 URL에 세 저장소 항목 추가, 각 폴더 origin 자동 조회 및 표시용 URL 저장. PM 질문 마커를 새 세션 초기 지시에 추가하고 웹에 미확인 버튼·스크롤 모달·PM 대화 이동 구현.
- 임시 폴더 독립 Git 루트와 origin 보존 등 Python 테스트 45개 통과. 실제 모델 질문 출력 검증은 미실행.
