...
프로젝트 URL 저장 시 HTTP 응답을 자동 확인합니다. 인증 없이 조회하므로 비공개 링크는 권한 필요 또는 확인 불가로 표시될 수 있습니다. 프로젝트 문서 창에서 편집 → Markdown 수정 → 저장할 수 있습니다. 다른 편집자가 먼저 저장한 경우 충돌을 알리고 입력 내용을 유지합니다.

프로젝트 문서는 5초마다 변경을 확인합니다. 최초 접속 이후 추가·수정된 문서는 상단 프로젝트 문서 버튼과 목록에 빨간 점으로 표시됩니다. 해당 문서를 열면 읽음 처리되며, 편집 중 입력은 자동 갱신하지 않습니다. 읽음 기록은 프로젝트별로 브라우저에 저장합니다.

프로젝트 관리 → 기술스택에서 Frontend·Backend·App·공통 환경의 언어, 프레임워크, 버전, 선택 이유를 관리합니다. 저장 내용은 `docs/architecture/TECH_STACK.md`에 바로 반영되며 문서 편집 창과 동일한 파일을 사용합니다. PM과 각 역할은 새 작업 전에 이 문서를 확인합니다.

프로젝트 관리 → 디자인 프롬프트에서 원하는 화면·분위기·색상·폰트·참고 링크를 입력하고 저장합니다. `docs/product/DESIGN_PROMPT.md`에 반영되며 프로젝트 문서에서도 편집할 수 있습니다. 저장만으로 디자인 작업을 시작하지는 않으며 PM에게 작업을 요청하세요.
