...
def prompt_for(role):
    ...
    return (
        ...
        f'제품 저장소는 {PRODUCT_ROOT / "frontend"}, {PRODUCT_ROOT / "app"}, {PRODUCT_ROOT / "backend"}로 분리된다. '
        '프론트엔드와 App의 구체적 용도는 PM이 사용자와 확인하고 각 폴더에서 Git 작업한다. '
        f'협업 문서·설정·작업 트리는 이 작업 경로 안에서만 관리하고 제품 루트에 복사하지 마라. '
        'Git 작업은 변경된 제품 폴더의 독립 저장소에서 수행하며 협업 도구 저장소와 혼동하지 마라. '
        f'docs/README.md, docs/agents/{document}.md, '
        'docs/issues/ACTIVE.md, docs/agents/TEAM_SETUP.md, docs/architecture/TECH_STACK.md를 읽어라. '
        'PM과 디자이너는 디자인 작업 전 docs/product/DESIGN_PROMPT.md를 다시 읽고 사용자 의도와 제약을 반영한다. '
        '기술스택의 원본은 docs/architecture/TECH_STACK.md이다. 새 작업 시작 전 다시 읽고 선택한 기술과 버전을 따른다. '
        '미정인 기술은 PM이 사용자와 확인하며 문서 변경만으로 패키지 설치나 기존 코드 마이그레이션을 시작하지 않는다. '
        ...
        'PM이 사용자에게 질문할 때 각 질문을 별도 줄에 [PM_QUESTION]질문 내용[/PM_QUESTION] 형식으로 출력한다. '
